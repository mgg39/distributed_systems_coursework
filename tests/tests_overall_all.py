...
def network_failure_packet_delay(test_id, f):
    try:
        ...
        result = client1.append_file("file_1", "A")
        log_to_file_and_console(f"{test_id} - Client 1 appended 'A' to file_1", f)
        ...
        # Give some time for lock release propagation
        time.sleep(2)
        ...
        if lock_acquired_client2:
            ...
            result = client2.append_file("file_1", "B")
            log_to_file_and_console(f"{test_id} - Client 2 appended 'B' to file_1", f)
        else:
...
def run_test(test_function, test_name, iterations=5):  # Set default iterations to 5
...
def main():
    ...
    with open(summary_file, "a") as summary:
        ...
        tests = [
            ...
            (network_failure_packet_drop_server_loss, "network_failure_packet_drop_server_loss"),
            (network_failure_packet_drop_client_loss, "network_failure_packet_drop_client_loss"),
            (network_failure_duplicated_packets, "network_failure_duplicated_packets"),
            (network_failure_combined_failures, "network_failure_combined_failures"),
        ...
        for test_func, test_name in tests:
            run_test(test_func, test_name, iterations=5)
